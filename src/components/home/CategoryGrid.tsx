  ...
  return (
    <section className="max-w-shell mx-auto px-5 py-12">
      <div className="flex items-end justify-between mb-7">
        <div>
          <div className="text-[12px] font-semibold tracking-[3px] uppercase text-ink-500 mb-2">{home.browseByCat as string}</div>
          <h2 className="text-[28px] md:text-[32px] font-bold text-ink-900 leading-tight">{home.exploreCategories as string}</h2>
        </div>
      </div>
        ...
        {CATEGORY_THEMES.map((c) => {
          ...
          return (
            <button
              ...
              className="group relative h-[280px] md:h-[340px] rounded-2xl overflow-hidden text-left text-white"
              style={{ background: c.bg }}
            >
              <div className="relative h-full flex flex-col justify-between p-7 md:p-9">
                <div>
                  <div className="text-[11px] tracking-[3px] uppercase text-white mb-3">{home.catOfItems ? home.catOfItems(count) : `${count} items`}</div>
                  <div className="text-[36px] md:text-[44px] font-bold leading-none mb-3">{label}</div>
                  <div className="text-[14px] text-white max-w-[280px] leading-relaxed">{descMap[c.key] ?? ''}</div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-[80px] opacity-40">{c.emoji}</div>
                </div>
              </div>
            </button>
          );
