            ...
            {agents.length === 0 ? (
              ...
              agents.map((a: any) => (
                <tr key={a.id} className="border-t border-neutral-100">
                  ...
                  <td className="px-3 py-2">{a.nickname} <span className="text-neutral-400">@{a.username}</span></td>
                  <td className="px-3 py-2 font-bold">{usd(a.balance)}</td>
                  <td className="px-3 py-2"><input id={`own_${a.id}`} defaultValue={a.own_rate} className="border border-neutral-200 rounded px-1.5 py-1 w-[64px]" /> %</td>
                  <td className="px-3 py-2"><input id={`sub_${a.id}`} defaultValue={a.sub_rate} className="border border-neutral-200 rounded px-1.5 py-1 w-[64px]" /> %</td>
                  <td className="px-3 py-2"><input id={`lim_${a.id}`} defaultValue={a.sub_rate_limit} className="border border-neutral-200 rounded px-1.5 py-1 w-[64px]" /> %</td>
                  <td className="px-3 py-2 max-w-[160px] truncate text-[11px]">{a.withdraw_address || '—'}</td>
                  <td className="px-3 py-2">
                    <button onClick={() => save(a.id)} className="px-2 py-0.5 text-[11px] rounded bg-emerald-600 text-white hover:bg-emerald-700">{t.admin.agents.save}</button>
                  </td>
                </tr>
              ))
            )}
